...
function save(){const p=state.pages[state.page];p.text=text.value;p.date=date.value;localStorage.setItem(KEY,JSON.stringify(state));$('#pageNo').textContent=`Página ${state.page+1}`}
...
function beginStickerMove(e,s,el){if(e.target.closest('.resize-handle,.sticker-remove'))return;e.preventDefault();selectedStickerId=s.id;renderStickers();const target=[...$('#stickerLayer').children].find(x=>x.style.left===s.x+'%'&&x.style.top===s.y+'%')||el;const start={x:e.clientX,y:e.clientY,left:s.x,top:s.y};target.setPointerCapture?.(e.pointerId);const move=ev=>{const r=paper.getBoundingClientRect();s.x=clamp(start.left+(ev.clientX-start.x)/r.width*100,0,100-s.width);s.y=clamp(start.top+(ev.clientY-start.y)/r.height*100,0,100-s.height);target.style.left=s.x+'%';target.style.top=s.y+'%'};const end=()=>{target.removeEventListener('pointermove',move);target.removeEventListener('pointerup',end);save()};target.addEventListener('pointermove',move);target.addEventListener('pointerup',end)}
...
function load(){const p=state.pages[state.page];text.value=p.text||'';date.value=p.date||'';text.style.color=p.textColor||'#111111';text.style.fontWeight=p.bold?'700':'400';$('#boldBtn').classList.toggle('active',!!p.bold);save();redraw();renderStickers();activateTextMode()}
...
$('#textBtn').onclick=activateTextMode;$$('[data-text-color]').forEach(b=>b.onclick=()=>{const p=state.pages[state.page];p.textColor=b.dataset.textColor;text.style.color=p.textColor;save()});$('#boldBtn').onclick=()=>{const p=state.pages[state.page];p.bold=!p.bold;text.style.fontWeight=p.bold?'700':'400';$('#boldBtn').classList.toggle('active',p.bold);save();text.focus()};text.oninput=save;date.onchange=save;$('#undo').onclick=()=>{
  text.focus();
  try{document.execCommand('undo')}catch(e){}
  state.pages[state.page].text=text.value;
  save();
};
...
if(v38Pdf)v38Pdf.onclick=()=>{
  ...
  const pages=(state.pages||[]).map(p=>`<section><div class="date">${esc(p.date||'')}</div><div>${esc(p.text||'').replace(/\n/g,'<br>')}</div></section>`).join('');
...
// V39 — resaltador para texto digitado
let v39Range=null;
const editor=text;
const hlBtn=document.getElementById('textHighlightBtn'), hlPal=document.getElementById('textHighlightPalette');
function rememberSelection(){const sel=getSelection();if(sel&&sel.rangeCount)v39Range=sel.getRangeAt(0).cloneRange();}
if(editor) editor.addEventListener('mouseup',rememberSelection);
if(editor) editor.addEventListener('keyup',rememberSelection);
if(hlBtn) hlBtn.onclick=()=>hlPal.classList.toggle('hidden');
document.querySelectorAll('#textHighlightPalette [data-hl]').forEach(b=>{
 b.onmousedown=e=>e.preventDefault();
 b.onclick=()=>{
   if(v39Range){const sel=getSelection();sel.removeAllRanges();sel.addRange(v39Range)}
   editor.focus();
   try{document.execCommand('hiliteColor',false,b.dataset.hl)}catch(e){try{document.execCommand('backColor',false,b.dataset.hl)}catch(_){}}
   hlPal.classList.add('hidden'); save();
 };
});
